...
def run(input_file, output_file, max_common_frames, n_clusters, version):
    ...
    gallery = [gallery[x] for x in ids]
    clusters, centers = cop_kmeans(dataset=data[:,2:], initialization=centers_init,  k=n_clusters, ml=must_link, cl=cannot_link, spherical=True)#, gallery=gallery)
